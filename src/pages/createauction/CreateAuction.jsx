    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
            try {
                ...
                const contractNftAuction = new web3.eth.Contract(abiNFTAuction, nftAuction);
                const contractNFTNumber = new web3.eth.Contract(abiNFTNumber, nftNumber);
                await contractNFTNumber.methods.approve(nftAuction, tokenId).send({from: account})
                await contractNftAuction.methods.createAuction(tokenId, price).send({from: account})
            } catch (error) {
                console.error(error);
            }
    };
